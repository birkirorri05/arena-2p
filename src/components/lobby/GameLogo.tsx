// SVG illustrations for each game card banner.
// All use viewBox="0 0 80 56" with white/semi-transparent elements
// so they blend over any colored gradient background.
    ...
    <svg viewBox="0 0 80 56" className="w-full h-full">
      {tiles.map(({ x, letter, pts }) => (
        <g key={letter}>
          ...
          <text x={x+8.5} y={25} textAnchor="middle" fontSize={11} fontWeight="bold"
            fill="#78350f" fontFamily="serif">{letter}</text>
          <text x={x+14} y={27} textAnchor="middle" fontSize={5}
            fill="#a16207">{pts}</text>
        </g>
      ))}
      {/* Second row of tiles (partial) */}
      {[
        { x:15, letter:"B", pts:3 },
        { x:33, letter:"L", pts:1 },
        { x:51, letter:"E", pts:1 },
        ...
        <g key={`b${letter}`}>
          ...
          <text x={x+8.5} y={45} textAnchor="middle" fontSize={11} fontWeight="bold"
            fill="#78350f" fontFamily="serif" opacity={0.6}>{letter}</text>
        </g>
      ))}
    </svg>
  );
...
function BackgammonLogo() {
  const topTri = [4,14,24,34,44,54];
  const botTri = [4,14,24,34,44,54];
  return (
    <svg viewBox="0 0 80 56" className="w-full h-full">
      {/* Board outline */}
      ...
      {topTri.map((x, i) => (
        <polygon key={`t${i}`} points={`${x+1},3 ${x+9},3 ${x+5},24`}
          fill={i%2===0 ? W : D} opacity={i%2===0 ? 0.85 : 0.5}/>
      ))}
      {/* Bottom triangles (pointing up) */}
      {botTri.map((x, i) => (
        <polygon key={`b${i}`} points={`${x+1},53 ${x+9},53 ${x+5},32`}
          fill={i%2===0 ? D : W} opacity={i%2===0 ? 0.5 : 0.85}/>
      ))}
      {/* Pieces */}
  ...
  return (
    <svg viewBox="0 0 80 56" className="w-full h-full">
      ...
      {Array.from({length:size}, (_,i) => (
        <g key={i}>
          <line x1={offset} y1={offset+i*spacing} x2={offset+(size-1)*spacing} y2={offset+i*spacing}
            stroke={W} strokeWidth={0.8} opacity={0.4}/>
          <line x1={offset+i*spacing} y1={offset} x2={offset+i*spacing} y2={offset+(size-1)*spacing}
            stroke={W} strokeWidth={0.8} opacity={0.4}/>
        </g>
      ))}
      {/* Stones */}
      {stones.map(([c,r,color], i) => (
        <circle key={i}
          cx={offset+c*spacing} cy={offset+r*spacing} r={5}
          fill={color==="b" ? "rgba(0,0,0,0.85)" : W}
...
function ReversiLogo() {
  return (
    <svg viewBox="0 0 80 56" className="w-full h-full">
      ...
      {/* Grid lines (4×3) */}
      {[24,40,56].map(x => (
        <line key={x} x1={x} y1={4} x2={x} y2={52} stroke={W} strokeWidth={0.8} opacity={0.3}/>
      ))}
      {[20,36,52].map(y => (
        <line key={y} x1={8} y1={y} x2={72} y2={y} stroke={W} strokeWidth={0.8} opacity={0.3}/>
      ))}
      {/* Center 4 discs (Reversi starting position) */}
...
function MancalaLogo() {
  ...
  return (
    <svg viewBox="0 0 80 56" className="w-full h-full">
      ...
      {/* Top row pits */}
      {pits.map((x, i) => (
        <ellipse key={`t${i}`} cx={x-6} cy={22} rx={5} ry={5} fill={W} opacity={0.8}/>
      ))}
      {/* Bottom row pits */}
      {pits.map((x, i) => (
        <ellipse key={`b${i}`} cx={x-6} cy={38} rx={5} ry={5} fill={W} opacity={0.8}/>
      ))}
      {/* Seeds (dots in pits) */}
      {[8, 20, 32].map(x => (
        <circle key={x} cx={x} cy={22} r={1.5} fill="rgba(0,0,0,0.4)"/>
      ))}
      {[44, 56, 68].map(x => (
        <circle key={x} cx={x} cy={38} r={1.5} fill="rgba(0,0,0,0.4)"/>
      ))}
    </svg>
  );
}
